...
# csv에서 파일 가져오기
data_gender = pd.read_csv('./csv/SNS-성별.csv', encoding='cp949', low_memory=False)
# print(data_gender)  # [16 rows x 5 columns]
data_age = pd.read_csv('./csv/SNS-연령.csv', encoding='cp949', low_memory=False)
# print(data_age)  # [40 rows x 5 columns]
data_area = pd.read_csv('./csv/SNS-지역.csv', encoding='cp949', low_memory=False)
...
for _, item in data_gender.iterrows():
    print(item)
    insert_query = "INSERT INTO snsGender (gender, name, total, year) VALUES (%s, %s, %s, %s)"
    cursor.execute(insert_query, (item['gender'], item['name'], item['total'], item['year']))
    conn.commit()

# SNS-연령 테이블에 데이터 삽입
for _, item in data_age.iterrows():
    insert_query = "INSERT INTO snsAge (age, name, total, year) VALUES (%s, %s, %s, %s)"
...
for _, item in data_area.iterrows():
    insert_query = "INSERT INTO snsArea (area, name, total, year) VALUES (%s, %s, %s, %s)"
